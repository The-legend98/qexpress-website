...
const content = {
  ar: {
    ...
    mainServices: [
      { color: "green", title: "التوصيل السريع", desc: "توصيل سريع ومحدد الوقت من الباب للباب للوثائق والطرود داخل سوريا وإلى وجهات دولية.", features: ["تغطية كل المحافظات السورية", "شحن دولي متكامل", "تتبع لحظي من الاستلام للتسليم", "توصيل آمن للوثائق الرسمية", "خدمة باب لباب"] },
      { color: "maroon", title: "اللوجستيات التعاقدية", desc: "حلول متكاملة تشمل التخزين وإدارة المخزون وتلبية الطلبات والتوزيع المخصص.", features: ["17,000 م² مساحة تخزين آمنة", "إدارة مخزون ذكية", "تلبية طلبات التجارة الإلكترونية", "خدمات توزيع مخصصة", "تقارير وتحليلات دورية"] },
    ],
  ...
  en: {
    ...
    mainServices: [
      { color: "green", title: "Express Delivery", desc: "Time-definite, door-to-door delivery for documents and parcels across Syria and to international destinations.", features: ["All Syria Governorates Coverage", "Fully Integrated International Shipping", "Real-time End-to-End Tracking", "Secure Official Document Delivery", "Door-to-Door Service"] },
      { color: "maroon", title: "Contract Logistics", desc: "Integrated solutions including warehousing, inventory management, order fulfillment, and dedicated distribution.", features: ["17,000 m² Secure Storage Area", "Smart Inventory Management", "E-Commerce Order Fulfillment", "Dedicated Distribution Services", "Periodic Reports & Analytics"] },
    ],
...
export default function ServicesPage() {
  ...
  return (
    ...
    >
        ...
        <div className="relative max-w-7xl mx-auto px-6 text-center">
          {heroIn.visible && (
            <>
              ...
              <h1 className={`text-4xl md:text-6xl font-black mb-6 tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}>
              ...
              <p className={`text-lg max-w-2xl mx-auto leading-relaxed ${isDark ? "text-slate-400" : "text-gray-500"}`} style={a("qe-fadeup", 0.4)}>
                {t.hero.desc}
              </p>
            </>
          )}
      ...
      <section
        ref={mainIn.ref}
        className={`py-24 relative z-10 ${isDark ? "bg-[#080d14]" : "bg-white"}`}
      >
      ...
      <section
        ref={subIn.ref}
        className={`py-24 ${isDark ? "bg-[#050810]" : "bg-gray-50"}`}
      >
        <div className="max-w-7xl mx-auto px-6">
          <h2
            className={`text-3xl md:text-5xl font-black text-center mb-20 tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}
            style={subIn.visible ? a("qe-fadeup", 0) : { opacity: 0 }}
          >
            {t.subTitle}
          </h2>
          <div className="grid md:grid-cols-2 gap-8">
            {t.subServices.map((s, i) => {
              ...
              return (
                <div
                  key={s.title}
                  className={`rounded-[2rem] p-10 border transition-all duration-500 group flex flex-col sm:flex-row gap-8 hover:shadow-2xl ${
                    isDark
                ...
                >
                  <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 ${
                    isGreen ? "bg-[#1a5c2a]/10 text-[#1a5c2a]" : "bg-[#8B1A2A]/10 text-[#8B1A2A]"
                  }`}>
                  ...
                  <div>
                    ...
                    <p className={`text-lg leading-relaxed font-medium ${isDark ? "text-slate-400" : "text-gray-500"}`}>{s.desc}</p>
                  </div>
                </div>
              );
            })}
      ...
      <section
        ref={ctaIn.ref}
        className={`py-24 relative overflow-hidden ${isDark ? "bg-[#080d14]" : "bg-gray-50"}`}
      >
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative">
          {ctaIn.visible && (
            <div style={a("qe-fadeup", 0)}>
              <h2 className={`text-4xl font-black mb-6 ${isDark ? "text-white" : "text-gray-900"}`}>{t.cta.title}</h2>
              <p className={`text-lg mb-10 max-w-xl mx-auto ${isDark ? "text-slate-400" : "text-gray-500"}`}>{t.cta.desc}</p>
              <Link
                href="/contact"
