...
function HeroGallery({ isAr }: { isAr: boolean }) {
  ...
  return (
    <div className="relative w-full h-full rounded-3xl overflow-hidden">
      ...
      <button onClick={nxt} aria-label="next" className="absolute top-1/2 -translate-y-1/2 end-3 z-10 w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition-transform" style={{ background:M, boxShadow:`0 4px 14px ${M}55` }}>
...
function StatBadge({ num, label, active, color, isDark }: { num:string; label:string; active:boolean; color:"g"|"m"; isDark:boolean }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <span className={`text-xs mt-0.5 font-semibold tracking-widest uppercase ${isDark ? "text-slate-500" : "text-gray-400"}`}>{label}</span>
    </div>
  );
}
...
export default function AboutPage() {
  ...
  const vdr = isAr ? "translateX(52px)"  : "translateX(-52px)";
  ...
  return (
    ...
    >
      ...
      {/* ══ HERO ══ */}
      <section
        className={`relative overflow-hidden pt-20 pb-28 md:pt-28 md:pb-36 ${isDark ? "bg-[#050810]" : "bg-white"}`}
        ref={heroIn.ref}
      >
        <div className="absolute inset-0 pointer-events-none" style={{
          backgroundImage:`linear-gradient(${M}18 1px,transparent 1px),linear-gradient(90deg,${G}14 1px,transparent 1px)`,
          backgroundSize:"60px 60px", animation:"qe-gridmov 14s linear infinite",
          opacity: isDark ? 0.1 : 0.4,
        }}/>
        <div className="absolute pointer-events-none" style={{ width:480, height:480, borderRadius:"50%", top:-200, [isAr?"left":"right"]:-160, background:`radial-gradient(circle at 60% 40%,${M}12 0%,${M}06 40%,transparent 70%)`, border:`1.5px solid ${M}15` }}/>
        <div className="absolute pointer-events-none" style={{ width:320, height:320, borderRadius:"50%", top:-100, [isAr?"left":"right"]:-60, border:`1px solid ${M}12` }}/>
        <div className="absolute pointer-events-none" style={{ width:360, height:360, borderRadius:"50%", bottom:-140, [isAr?"right":"left"]:-120, background:`radial-gradient(circle,${G}10 0%,transparent 65%)`, border:`1.5px solid ${G}12` }}/>
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-14 items-center">
                ...
                <h1 className="text-5xl md:text-[3.5rem] font-black leading-[1.05] tracking-tight mb-4">
                  {t.hero.title.split(" ").map((w,i)=>(
                    <span key={i} className="inline-block me-2.5" style={a("qe-vanin",0.07+i*0.07,0.62)}>
                      <span style={{color: i%2===0 ? (isDark ? "#ffffff" : "#1a0a0e") : M}}>{w}</span>
                    </span>
                  ))}
                </h1>
                <p className="text-lg font-bold mb-5" style={{color:G,...a("qe-vanin",0.38,0.6)}}>{t.hero.subtitle}</p>
                <p className={`text-base leading-relaxed mb-10 max-w-md ${isDark ? "text-slate-400" : "text-[#3a1a1e]/70"}`} style={a("qe-fadeup",0.45,0.65)}>
                  {t.hero.desc}
                </p>
                <div className={`flex gap-10 pt-8 border-t ${isDark ? "border-white/8" : ""}`} style={isDark ? {...a("qe-fadeup",0.55,0.65)} : {borderColor:`${M}15`,...a("qe-fadeup",0.55,0.65)}}>
                  {t.hero.stats.map((s,i)=>(
                    <StatBadge key={s.label} num={s.num} label={s.label} active={heroIn.visible} color={i===1?"m":"g"} isDark={isDark}/>
                  ))}
                </div>
              </>}
            </div>
            ...
            <div className="relative hidden md:block" style={heroIn.visible?a("qe-scaleup",0.1,0.82):{opacity:0}}>
              ...
              <div className="absolute -top-6 -end-6 w-14 h-14 rounded-full border-2 flex items-center justify-center" style={{borderColor:`${M}35`,animation:"qe-float 5s ease-in-out infinite"}}>
                <div className="w-5 h-5 rounded-full" style={{background:`${M}60`}}/>
              </div>
              <div className="absolute -bottom-4 -start-4 w-10 h-10 rounded-full border-2" style={{borderColor:`${G}35`,animation:"qe-float 7s ease-in-out infinite reverse"}}/>
            </div>
          </div>
        </div>
      </section>
      ...
      >
        <div className="absolute pointer-events-none" style={{ width:500, height:500, borderRadius:"50%", top:-200, [isAr?"right":"left"]:-200, background:`radial-gradient(circle,${G}08 0%,transparent 60%)`, border:`1px solid ${G}10` }}/>
        <div className="absolute pointer-events-none" style={{ width:300, height:300, borderRadius:"50%", bottom:-100, [isAr?"left":"right"]:-80, background:`radial-gradient(circle,${M}07 0%,transparent 65%)`, border:`1px solid ${M}10` }}/>
        ...
        <div className="max-w-7xl mx-auto px-6">
          {missionIn.visible && (
            <div className="text-center mb-14" style={a("qe-fadeup",0,0.6)}>
              <span className="qe-badge" style={{background:`${M}${isDark?"25":"12"}`,color: isDark ? "#e05568" : M}}>
                <span className="w-1.5 h-1.5 rounded-full" style={{background: isDark ? "#e05568" : M}}/>
                {t.mission.badge}
              </span>
            </div>
          )}
          ...
          <div className="grid md:grid-cols-2 gap-7">
            <TiltCard glowColor={G}
              ...
              style={{
                background: isDark ? "#0d1421" : "#fff",
                borderColor: isDark ? `${G}25` : `${G}20`,
                ...(missionIn.visible?a("qe-rotatein",0.1,0.75):{opacity:0}),
              }}>
              <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage:`radial-gradient(circle at 2px 2px,${G} 1.5px,transparent 0)`, backgroundSize:"22px 22px" }}/>
              <div className="absolute top-0 start-0 w-20 h-20 pointer-events-none" style={{ background:`radial-gradient(circle at 0% 0%,${G}18 0%,transparent 70%)` }}/>
              <div className="relative mb-7">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5" style={{background:`${G}${isDark?"20":"10"}`}}>
                  <svg className="w-7 h-7" style={{color: isDark ? "#4ade80" : G}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                  </svg>
                </div>
                <h3 className={`text-2xl font-black mb-3 ${isDark ? "text-white" : ""}`} style={isDark ? {} : {color:"#1a0a0e"}}>{t.mission.vision.title}</h3>
              ...
              <p className={`relative leading-relaxed text-[15px] ${isDark ? "text-slate-400" : "text-gray-500"}`}>{t.mission.vision.desc}</p>
            </TiltCard>
            ...
            <TiltCard glowColor={M}
              ...
              <div className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage:"radial-gradient(circle at 2px 2px,white 1.5px,transparent 0)", backgroundSize:"24px 24px" }}/>
              <div className="absolute bottom-0 end-0 w-40 h-40 pointer-events-none" style={{ background:`radial-gradient(circle at 100% 100%,${G}25 0%,transparent 70%)` }}/>
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          {valuesIn.visible && (
            <div className="text-center mb-14" style={a("qe-fadeup",0,0.6)}>
              <span className="qe-badge" style={{background:`${G}${isDark?"25":"10"}`,color: isDark ? "#4ade80" : G}}>
                <span className="w-1.5 h-1.5 rounded-full" style={{background: isDark ? "#4ade80" : G}}/>{t.values.badge}
              </span>
              <h2 className={`text-4xl md:text-5xl font-black mt-5 tracking-tight ${isDark ? "text-white" : ""}`} style={isDark ? {} : {color:"#1a0a0e"}}>{t.values.title}</h2>
            </div>
          )}
                ...
                <TiltCard key={item.title} glowColor={ac}
                  ...
                  style={{
                    background: isDark
                      ? (i%2!==0 ? `${M}10` : "#0d1421")
                      : (i%2!==0 ? `${M}05` : "#f9fafb"),
                    borderColor: isDark ? `${ac}20` : "#e5e7eb",
                  ...
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5 relative" style={{background:`${ac}${isDark?"20":"12"}`,color: isDark ? (ac === G ? "#4ade80" : "#e05568") : ac}}>
                    {VALUE_ICONS[i]}
                    <div className="absolute inset-0 rounded-xl border" style={{borderColor:`${ac}25`}}/>
                  </div>
                  <div className="text-[10px] font-black tracking-[.2em] mb-3" style={{color:`${ac}${isDark?"80":"55"}`}}>0{i+1}</div>
                  <h3 className="font-black text-sm mb-2" style={{color:ac}}>{item.title}</h3>
                  <p className={`text-sm leading-relaxed ${isDark ? "text-slate-500" : "text-gray-400"}`}>{item.desc}</p>
      ...
      <section
        className="py-24 relative overflow-hidden"
        ref={techIn.ref}
        style={isDark ? {background:"#080d14"} : {background:`linear-gradient(150deg,#f0f9f3 0%,#fdf3f4 50%,#f0f9f3 100%)`}}
      >
        <div className="absolute pointer-events-none" style={{ width:440, height:440, borderRadius:"50%", top:-120, [isAr?"left":"right"]:-120, background:`radial-gradient(circle,${M}10 0%,transparent 65%)`, border:`1px solid ${M}12` }}/>
        <div className="absolute pointer-events-none" style={{ width:360, height:360, borderRadius:"50%", bottom:-100, [isAr?"right":"left"]:-80, background:`radial-gradient(circle,${G}10 0%,transparent 65%)`, border:`1px solid ${G}12` }}/>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          {techIn.visible && (
            <div className="text-center mb-16" style={a("qe-fadeup",0,0.6)}>
              <span className="qe-badge" style={{background:`${G}${isDark?"25":"12"}`,color: isDark ? "#4ade80" : G}}>
                <span className="w-1.5 h-1.5 rounded-full" style={{background: isDark ? "#4ade80" : G}}/>{t.tech.badge}
              </span>
              <h2 className={`text-4xl md:text-5xl font-black mt-5 tracking-tight ${isDark ? "text-white" : ""}`} style={isDark ? {} : {color:"#1a0a0e"}}>{t.tech.title}</h2>
            </div>
          )}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {t.tech.items.map((item,i) => {
              ...
              return (
                <div
                  key={item.title}
                  className={`rounded-2xl p-7 border-2 flex gap-6 cursor-default relative overflow-hidden group transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${isDark ? "bg-[#0d1421]" : "bg-white"}`}
                  style={{
                    borderColor:`${ac}${isDark?"25":"12"}`,
                ...
                >
                  <div className="shrink-0">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm"
                      style={{background:`${ac}${isDark?"20":"10"}`,border:`1.5px solid ${ac}30`,color: isDark ? (ac===G?"#4ade80":"#e05568") : ac}}>
                      0{i+1}
                    </div>
                  </div>
                  <div className="relative z-10">
                    <h3 className={`font-black text-base mb-2 ${isDark ? "text-white" : ""}`} style={isDark ? {} : {color:"#1a0a0e"}}>{item.title}</h3>
                    <p className={`text-sm leading-relaxed ${isDark ? "text-slate-500" : "text-gray-500"}`}>{item.desc}</p>
                  </div>
                  <div className="absolute bottom-0 end-0 w-24 h-24 rounded-tl-3xl pointer-events-none opacity-60 group-hover:opacity-100 transition-opacity duration-300"
